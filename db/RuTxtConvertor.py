...
class RuTxtConvertor:
    ...
    def convert_cells_to_nameid_dict(self, cells: List[str], geo_item: GeoInfo) -> List[Dict[str, Union[str, GeoInfo]]]:
        all_str_names_to_db = []
        names = self.__all_names_in_str(cells)

        for name in names:
            all_str_names_to_db.append(dict(name=name, idlnk=geo_item))

        return all_str_names_to_db

    @staticmethod
    def __all_names_in_str(cells: List[str]) -> List[str]:
        names = [cells[1]]
